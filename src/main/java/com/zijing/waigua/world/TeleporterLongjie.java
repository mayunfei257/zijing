...
public class TeleporterLongjie extends Teleporter{
	private final WorldServer worldServer;
	private final Random random;
	private final Long2ObjectMap<Teleporter.PortalPosition> destinationCoordinateCache = new Long2ObjectOpenHashMap(4096);
	private static final String __OBFID = "CL_00000153";

	public TeleporterLongjie(WorldServer world) {
		super(world);
			...
			for (int l = -2; l <= 2; ++l) {
				for (int i1 = -2; i1 <= 2; ++i1) {
					for (int j1 = -1; j1 < 3; ++j1) {
						...
						int i2 = k + i1 * b1 - l * b0;
						boolean flag = j1 < 0;
						this.worldServer.setBlockState(new BlockPos(k1, l1, i2), flag ? Blocks.GOLD_BLOCK.getDefaultState() : Blocks.AIR.getDefaultState());
					}
				}
			}
	...
	public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_) {
		...
		if (this.destinationCoordinateCache.containsKey(k)) {
			...
			for (int l = -128; l <= 128; ++l) {
				...
				for (int i1 = -128; i1 <= 128; ++i1) {
					for (BlockPos blockpos = blockpos4.add(l, this.worldServer.getActualHeight() - 1 - blockpos4.getY(), i1); blockpos
							.getY() >= 0; blockpos = blockpos1) {
						blockpos1 = blockpos.down();

						if (this.worldServer.getBlockState(blockpos).getBlock() == BaseControl.blockPortalLongjie) {
							while (this.worldServer.getBlockState(blockpos1 = blockpos.down()).getBlock() == BaseControl.blockPortalLongjie) {
								blockpos = blockpos1;
							}
		...
		if (d0 >= 0.0D) {
			if (flag1) {
				this.destinationCoordinateCache.put(k, new Teleporter.PortalPosition(object, this.worldServer.getTotalWorldTime()));
			}
			...
			if (this.worldServer.getBlockState(((BlockPos) object).west()).getBlock() == BaseControl.blockPortalLongjie) {
				enumfacing = EnumFacing.NORTH;
			}

			if (this.worldServer.getBlockState(((BlockPos) object).east()).getBlock() == BaseControl.blockPortalLongjie) {
				enumfacing = EnumFacing.SOUTH;
			}

			if (this.worldServer.getBlockState(((BlockPos) object).north()).getBlock() == BaseControl.blockPortalLongjie) {
				enumfacing = EnumFacing.EAST;
			}

			if (this.worldServer.getBlockState(((BlockPos) object).south()).getBlock() == BaseControl.blockPortalLongjie) {
	...
	private boolean func_180265_a(BlockPos p_180265_1_) {
		return !this.worldServer.isAirBlock(p_180265_1_) || !this.worldServer.isAirBlock(p_180265_1_.up());
	}
	...
	public boolean makePortal(Entity p_85188_1_) {
							...
							for (i4 = 0; i4 < 3; ++i4) {
								for (j4 = 0; j4 < 4; ++j4) {
									for (k4 = -1; k4 < 4; ++k4) {
										...
										Block tmp = this.worldServer.getBlockState(new BlockPos(l4, i5, j5)).getBlock();
										if (k4 < 0 && !tmp.getMaterial(tmp.getDefaultState()).isSolid() || k4 >= 0
												&& !this.worldServer.isAirBlock(new BlockPos(l4, i5, j5))) {
											continue label271;
										}
									}
								}
							...
							for (j3 = l1; j3 < l1 + 2; ++j3) {
								...
								for (i4 = 0; i4 < 4; ++i4) {
									for (j4 = -1; j4 < 4; ++j4) {
										...
										l4 = i3 + j4;
										i5 = k2 + (i4 - 1) * l3;
										Block tmpb = this.worldServer.getBlockState(new BlockPos(k4, l4, i5)).getBlock();
										if (j4 < 0 && !tmpb.getMaterial(tmpb.getDefaultState()).isSolid() || j4 >= 0
												&& !this.worldServer.isAirBlock(new BlockPos(k4, l4, i5))) {
											continue label219;
										}
									}
								}
		...
		if (d0 < 0.0D) {
			i1 = MathHelper.clamp(i1, 70, this.worldServer.getActualHeight() - 10);
			j2 = i1;
			...
			for (i3 = -1; i3 <= 1; ++i3) {
				for (j3 = 1; j3 < 3; ++j3) {
					for (k3 = -1; k3 < 3; ++k3) {
						...
						j4 = k2 + (j3 - 1) * l2 - i3 * l5;
						boolean flag = k3 < 0;
						this.worldServer.setBlockState(new BlockPos(l3, i4, j4),
		...
		IBlockState iblockstate = BaseControl.blockPortalLongjie.getDefaultState().withProperty(BlockPortal.AXIS, l5 == 0 ? EnumFacing.Axis.Z : EnumFacing.Axis.X);
			...
			for (k3 = 0; k3 < 4; ++k3) {
				for (l3 = -1; l3 < 4; ++l3) {
					...
					this.worldServer.notifyNeighborsOfStateChange(new BlockPos(i4, j4, k4),
							this.worldServer.getBlockState(new BlockPos(i4, j4, k4)).getBlock(), true);
				}
			}
		}
