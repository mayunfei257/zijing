	...
	public static class TeleporterDimensionMod extends Teleporter {
		...
		public boolean placeInExistingPortal(Entity entityIn, float p_180620_2_) {
				...
				for (int l = -128; l <= 128; ++l) {
					...
					for (int i1 = -128; i1 <= 128; ++i1) {
						for (BlockPos blockpos = blockpos4.add(l, this.worldServerInstance.getActualHeight() - 1 - blockpos4.getY(), i1); blockpos
							...
							if (this.worldServerInstance.getBlockState(blockpos).getBlock() == BaseControl.blockPortalLongjie) {
								while (this.worldServerInstance.getBlockState(blockpos1 = blockpos.down()).getBlock() == BaseControl.blockPortalLongjie) {
									blockpos = blockpos1;
								}
			...
			if (d0 >= 0.0D) {
				...
				if (this.worldServerInstance.getBlockState(((BlockPos) object).west()).getBlock() == BaseControl.blockPortalLongjie) {
					enumfacing = EnumFacing.NORTH;
				}

				if (this.worldServerInstance.getBlockState(((BlockPos) object).east()).getBlock() == BaseControl.blockPortalLongjie) {
					enumfacing = EnumFacing.SOUTH;
				}

				if (this.worldServerInstance.getBlockState(((BlockPos) object).north()).getBlock() == BaseControl.blockPortalLongjie) {
					enumfacing = EnumFacing.EAST;
				}

				if (this.worldServerInstance.getBlockState(((BlockPos) object).south()).getBlock() == BaseControl.blockPortalLongjie) {
